...
import { AlertTriangle, TrendingUp, Car, Eye, Calendar, Clock, Database } from 'lucide-react'
...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function fetchMetrics() {
      try {
        ...
        const sanitizedData = {
          ...data,
          time_distribution: Array.isArray(data?.time_distribution) ? data.time_distribution : [],
  ...
  // Safely transform time distribution data
  ...
  const mostActiveTime = timeDistributionData.length > 0 
    ...
    <DashboardLayout>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <CardFooter className="flex-col items-start gap-2 text-sm">
                ...
                Most active time: {formatTimeRange(metrics.time_distribution.reduce((max, current) => 
                    current.frequency > max.frequency ? current : max,
                    metrics.time_distribution[0] || { frequency: 0, timeRange: '0-0' }
                ).timeRange)}
            ...
            <CardHeader>
                ...
                <CardDescription>Most frequently seen license plates in the last 24 hours</CardDescription>
            </CardHeader>
            <CardContent>
                {loading ? (
                ...
                <ul className="space-y-4">
                    ...
                    <li key={plate.plate} className="flex items-center justify-between p-2 rounded-lg bg-muted">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">Last seen: {formatTimestamp(plate.lastSeen)}</p>
                        </div>
                        </div>
                        <Badge variant={index === 0 ? "destructive" : "secondary"} className="ml-2">
                        {plate.count} reads
                        </Badge>
                    </li>
                    ))}
