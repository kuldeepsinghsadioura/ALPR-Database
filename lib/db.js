...
export async function getPool() {
  try {
    ...
    // If config has changed, reset the pool
    if (pool && newConfigHash !== currentConfigHash) {
      await resetPool();
    }

    // If pool doesn't exist or was reset, create a new one
    if (!pool) {
      pool = new pg.Pool({
        host: config?.database?.host?.split(":")[0] || "localhost",
        port: config?.database?.host
          ? parseInt(config.database.host.split(":")[1])
          : 5432,
  ...
  const query = `
    ...
    time_distributions AS (
      SELECT 
        ...
        COUNT(*) / 7 as frequency,  -- Average per day over 7 days
        COUNT(DISTINCT plate_number) / 7 as unique_vehicles
      FROM plate_reads 
      WHERE timestamp > NOW() - INTERVAL '7 days'
      GROUP BY hour_block
      ORDER BY hour_block
