    ...
    const result = await pool.query(
      ...
      WITH plate_data AS (
        ...
        GROUP BY 
          ...
          kp.notes
        ORDER BY ${
          sortBy === "days_since_last_seen" ? "MAX(pr.timestamp)" : sortBy
        } ${sortDesc ? "DESC" : "ASC"}
  ...
  try {
    ...
    if (count > maxRecords * 1.1) {
      const deleteResult = await pool.query(
        ...
        WHERE id IN (SELECT id FROM ranked_reads)
        RETURNING COUNT(*)
        `,
        [maxRecords]
      );

      console.log(`Cleaned up ${deleteResult.rows[0].count} old records`);
    }
  } catch (error) {
