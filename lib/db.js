...
export async function getRecordsToMigrate() {
  return withClient(async (client) => {
    const result = await client.query(`
...
export async function updateImagePaths(id, imagePath, thumbnailPath) {
  return withClient(async (client) => {
    await client.query(
      `
      UPDATE plate_reads 
...
export async function clearImageDataWithPathVerification() {
  return withClient(async (client) => {
    const result = await client.query(`
