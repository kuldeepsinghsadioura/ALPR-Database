  ...
  async generateOCRDataset() {
    try {
      ...
      for (let i = 0; i < records.length; i += batchSize) {
        ...
        for (const record of batch) {
          try {
            ...
            const outputFilePath = path.join(this.ocrPath, filename);

            await processedImage.toFile(outputFilePath);
            console.log(
              `\x1b[33m[AI TRAINING]\x1b[0m Saved processed image for record \x1b[36m${record.id}\x1b[0m to \x1b[32m${filename}\x1b[0m`
            );
      ...
      if (csvData.length > 0) {
        ...
        console.log(
          `\x1b[33m[AI TRAINING]\x1b[0m \x1b[32mSuccessfully processed ${csvData.length} OCR records\x1b[0m`
        );
      } else {
        console.warn(
          `\x1b[33m[AI TRAINING]\x1b[0m \x1b[33mNo records were successfully processed\x1b[0m`
        );
      }
    } catch (error) {
      console.error(
