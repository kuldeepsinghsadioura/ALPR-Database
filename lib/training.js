...
class TrainingDataGenerator {
  ...
  async initialize() {
    try {
      ...
      // Create license plate directories - simplified structure
  ...
  async processOCRImage(imageBuffer, coordinates) {
    try {
      ...
      // Create corners for four-point transform
      const points = [
        [x1, y1], // Top-left
        [x2, y1], // Top-right
        [x2, y2], // Bottom-right
      ...
      const transformedImage = await this.fourPointTransform(image, points);

      // Apply additional processing to enhance text visibility
      const processedImage = await transformedImage
      ...
      for (const record of records) {
        try {
          if (
            ...
            console.warn(
              `Skipping record ${record.id}: Invalid crop coordinates`
            );
            continue;
          }
          ...
          // Process the image with our enhanced OCR processing
  ...
  async generateLicensePlateDataset() {
    ...
    try {
      ...
      console.log(`Processing ${records.length} license plate records`);
      ...
      for (const record of records) {
        try {
          ...
          // Save image and annotation without considering validation status
          await fs.writeFile(
