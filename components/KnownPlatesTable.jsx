...
import { getPlates, getTags, addKnownPlate, tagPlate, untagPlate, deletePlate, fetchPlateInsights, getKnownPlatesList } from '@/app/actions'
      ...
      if (result.success) {
        setData(prevData => prevData.map(plate => 
          plate.plate_number === activePlate.plate_number 
            ? { ...plate, name: editPlateData.name, notes: editPlateData.notes }
            : plate
        ));
      ...
      <CardContent className="py-4">
                    ...
                    <TableRow key={plate.plate_number}>
                            ...
                            <div className="flex flex-wrap items-center gap-1.5">
                                ...
                                plate.tags.map((tagName) => {
                                    ...
                                    <Badge 
                                        ...
                                        style={{ backgroundColor: tagInfo.color, color: '#fff' }}
                                    >
                                        <span>{tagName}</span>
                                        <Button
                                        variant="ghost"
                                        ...
                                        onClick={() => handleRemoveTag(plate.plate_number, tagName)}
                                        >
                                        <X className="h-3 w-3" />
                        ...
                        <div className="flex justify-end space-x-2">
                            ...
                            onClick={() => {
                                ...
                                setEditPlateData({ name: plate.name, notes: plate.notes });
                            ...
                            >
                            <X className="h-4 w-4" />
                            <span className="sr-only">Remove from known plates</span>
                            </Button>
                        </div>
                        </TableCell>
                    </TableRow>
            ...
            <Dialog open={isEditPlateOpen} onOpenChange={setIsEditPlateOpen}>
                ...
                <div className="grid gap-4 py-4">
                    ...
                    <Input
                        ...
                        onChange={(e) => setEditPlateData({ ...editPlateData, name: e.target.value })}
                    ...
                    <Textarea
                        ...
                        onChange={(e) => setEditPlateData({ ...editPlateData, notes: e.target.value })}
                        className="col-span-3"
                    />
                    </div>
                </div>
                <DialogFooter>
                    <Button type="submit" onClick={handleEditPlate}>Update Plate Details</Button>
                </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <Dialog open={isRemoveConfirmOpen} onOpenChange={setIsRemoveConfirmOpen}>
                ...
                <DialogHeader>
                    ...
                    <DialogDescription>
                    Are you sure you want to remove {activePlate?.plate_number} from known plates?
                    This action can be undone by adding the plate back to known plates later.
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter>
                    <Button variant="outline" onClick={() => setIsRemoveConfirmOpen(false)}>Cancel</Button>
                    <Button variant="destructive" onClick={handleRemoveFromKnown}>Remove</Button>
                </DialogFooter>
                </DialogContent>
            </Dialog>
            </div>
